...
class QuizController extends AbstractController
{
  ...
  /**
   * @Route("/result", name="quiz_result", methods={"POST"})
   */
  public function calculateResult(Request $request, EntityManagerInterface $entityManager): Response
  {
    ...
    $categoryName = "Questions technique";
    ...
    // Create a new Resultat object and associate it with the user
    $resultat = new Resultat();
    $resultat->setScore($score); // Set the score value
    ...
    $pdfContent .= '</p>';
    $pdfContent .= 'Resultat de test géneral :';
    ...
    // Save the PDF file
    $pdfFileName = 'result_' . $resultat->getId() . '.pdf';
    $pdfFilePath = $this->getParameter('pdf_directory') . '/' . $pdfFileName;
    file_put_contents($pdfFilePath, $pdfContent);

    $resultat->setResultat($pdfFileName); // Set the PDF file name in the 'resultat' column
    $entityManager->flush();
  ...
  /**
   * @Route("/pdf/show/{filename}", name="pdf_show", methods={"GET"})
   */
